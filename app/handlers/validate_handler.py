...
TRAIN_COLUMNS = [
    'sex', 'age', 'side', 'BW', 'Ht', 'BMI', 
    'IKDC pre', 'IKDC 3 m', 'IKDC 6 m', 'IKDC 1 Y', 'IKDC 2 Y',
    'Lysholm pre', 'Lysholm 3 m', 'Lysholm 6 m', 'Lysholm 1 Y', 'Lysholm 2 Y',
    'Pre KL grade', 'Post KL grade 2 Y', 'MRI healing 1 Y',
    'MM extrusion pre', 'MM extrusion post'
]
...
def validate_predict_request(data: dict) -> bool:
    try:
        ...
        numeric_fields = [field for field, type_ in PredictRequest.__annotations__.items()
...
def validate_train_request(data: dict) -> bool:
    try:
        ...
        numeric_fields = [field for field, type_ in TrainRequest.__annotations__.items()
                          if type_ in (int, float) and field not in ['sex', 'age', 'side']]
